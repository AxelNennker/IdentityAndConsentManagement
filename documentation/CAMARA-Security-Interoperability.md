...
The subject identifier must be locally unique identifier ( for e.g. a pseudonymous customer reference) and a case-sensitive ASCII string with a maximum length of 255. The sub value MUST not contain the MSISDN. It is generated by the OpenID Connect Provier (OP).
